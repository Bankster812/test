...
def main() -> None:
    ...
    parser.add_argument('--llm-key', type=str, default='',            help='API key for any cloud LLM provider (auto-detected)')
    ...
    # ── Load persisted config (API keys stored in data/config.json, gitignored) ─
    ...
    if _cfg_path.exists():
        try:
            ...
            for _env, _key in [
                ...
                ("CEREBRAS_API_KEY",   _cfg.get("cerebras_api_key", "")),
                ("NVIDIA_API_KEY",     _cfg.get("nvidia_api_key", "")),
                ("OPENROUTER_API_KEY", _cfg.get("openrouter_api_key", "")),
            ]:
                if _key and not os.environ.get(_env):
                    os.environ[_env] = _key
        except Exception as _e:
            print(f"[brain_web] config.json load error: {_e}")

    # ── Cloud LLM — auto-detect whichever free key is available ─────────────────
    cloud_client = None
    if not args.no_llm:
        try:
            from neuromorphic.llm.generic_client import GenericLLMClient

            # Priority: NVIDIA → Groq → Gemini → Cerebras → OpenRouter
            _candidates = [
                ("NVIDIA Llama 3.3 70B","NVIDIA_API_KEY",     args.llm_key, GenericLLMClient.nvidia),
                ("Groq Llama 3.3 70B", "GROQ_API_KEY",       args.llm_key, GenericLLMClient.groq),
                ("Gemini 2.0 Flash",   "GEMINI_API_KEY",     args.llm_key, GenericLLMClient.gemini),
                ("Cerebras 70B",       "CEREBRAS_API_KEY",   args.llm_key, GenericLLMClient.cerebras),
                ("OpenRouter (free)",  "OPENROUTER_API_KEY", args.llm_key, GenericLLMClient.openrouter),
            ]
            for label, env_var, explicit_key, factory in _candidates:
                ...
                if c.is_available():
                    cloud_client = c
                    print(f'[brain_web] Cloud LLM: {label} ✓')
                    break

            if not cloud_client:
                print('[brain_web] Cloud LLM: no API key found.')
                print('  → Set one of: GEMINI_API_KEY / CEREBRAS_API_KEY / GROQ_API_KEY')
                print('  → Free keys:  aistudio.google.com  |  cloud.cerebras.ai  |  console.groq.com')
        except ImportError:
            pass
